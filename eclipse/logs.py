...
def parse_jsonl(path: Path) -> list[dict[str, Any]]:
    ...
    try:
        ...
        raise EclipseError(f"Journal illisible : {path} ({error})") from error
    for line in lines:
        try:
...
def system_logs(*, limit: int = 50, last: str = "1h") -> list[LogEntry]:
    ...
    try:
        ...
        raise EclipseError(f"Logs système indisponibles : {error}") from error
    if completed.returncode != 0:
        raise EclipseError(completed.stderr.strip() or "Logs système indisponibles.")
    rows: list[LogEntry] = []
...
def collect_logs(
    ...
    *,
    ...
    if unknown:
        raise EclipseError(f"Source log inconnue : {', '.join(unknown)}")
    rows: list[LogEntry] = []
...
def export_logs(entries: list[LogEntry], destination: Path) -> Path:
    ...
    try:
        ...
        raise EclipseError(f"Export logs impossible : {error}") from error
