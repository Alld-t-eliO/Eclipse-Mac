...
def validate_interval(value: str) -> str:
    if value not in INTERVALS:
        raise EclipseError("Intervalle automation invalide. Utilise hour, day ou week.")
    return value
...
def load_jobs(root: Path | None = None) -> dict[str, AutomationJob]:
    ...
    try:
        ...
        raise EclipseError(f"Registre automation illisible : {error}") from error
    if not isinstance(raw, list):
        raise EclipseError("Registre automation invalide : liste attendue.")
...
def save_jobs(jobs: dict[str, AutomationJob], root: Path | None = None) -> None:
    ...
    try:
        ...
        raise EclipseError(f"Impossible d'écrire le registre automation : {error}") from error
...
def add_job(
    ...
    *,
    ...
    if name in jobs and not overwrite:
        raise EclipseError(f"Automation déjà existante : {name}")
    job = AutomationJob(
        name=name,
        every=interval,
...
def set_enabled(name: str, enabled: bool, *, root: Path | None = None) -> AutomationJob:
    jobs = load_jobs(root)
    if name not in jobs:
        raise EclipseError(f"Automation inconnue : {name}")
    old = jobs[name]
