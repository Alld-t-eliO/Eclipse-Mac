...
def finding(level: str, category: str, title: str, detail: str = "", remediation: str = "") -> Finding:
    if level not in LEVEL_ORDER:
        raise EclipseError(f"Niveau de finding invalide : {level}")
...
def check_filevault() -> list[Finding]:
    ...
    if "FileVault is On" in text:
        return [finding("OK", "security", "FileVault activé", stdout)]
    if "FileVault is Off" in text:
        return [finding("WARNING", "security", "FileVault désactivé", text, "Active FileVault dans Réglages Système > Confidentialité et sécurité.")]
    ...
    if encrypted:
        return [finding("INFO", "security", "FileVault à vérifier", "\n".join(encrypted[:10]))]
    if code:
        return [finding("INFO", "security", "FileVault non vérifiable", text)]
    return [finding("INFO", "security", "FileVault à vérifier", text)]


def check_gatekeeper() -> list[Finding]:
    code, stdout, stderr = run_text(["spctl", "--status"])
    if code:
        return [finding("ERROR", "security", "Gatekeeper non vérifiable", stderr)]
    if "assessments enabled" in stdout:
        return [finding("OK", "security", "Gatekeeper activé", stdout)]
    return [finding("WARNING", "security", "Gatekeeper désactivé", stdout, "Réactive Gatekeeper avec spctl après revue de la configuration.")]


def check_sip() -> list[Finding]:
    code, stdout, stderr = run_text(["csrutil", "status"])
    if code:
        return [finding("ERROR", "security", "SIP non vérifiable", stderr)]
    if "enabled" in stdout.lower():
        return [finding("OK", "security", "System Integrity Protection activé", stdout)]
    return [finding("CRITICAL", "security", "System Integrity Protection désactivé", stdout, "Réactive SIP depuis recoveryOS avec csrutil enable.")]


def check_xprotect() -> list[Finding]:
    bundle = Path("/Library/Apple/System/Library/CoreServices/XProtect.bundle")
    if not bundle.exists():
        return [finding("INFO", "security", "XProtect non détecté au chemin standard")]
    _, version, _ = run_text(["defaults", "read", str(bundle / "Contents/Info"), "CFBundleShortVersionString"])
    return [finding("OK", "security", "XProtect détecté", version)]


def check_firewall() -> list[Finding]:
    tool = Path("/usr/libexec/ApplicationFirewall/socketfilterfw")
    if not tool.exists():
        return [finding("ERROR", "firewall", "Utilitaire firewall introuvable")]
    results: list[Finding] = []
    _, state, _ = run_text([str(tool), "--getglobalstate"])
    if "enabled" in state.lower():
        results.append(finding("OK", "firewall", "Firewall applicatif activé", state))
    else:
        results.append(finding("WARNING", "firewall", "Firewall applicatif désactivé", state, "Active le firewall dans Réglages Système > Réseau > Coupe-feu."))
    _, stealth, _ = run_text([str(tool), "--getstealthmode"])
    level = "OK" if "enabled" in stealth.lower() else "INFO"
    results.append(finding(level, "firewall", "Mode furtif", stealth))
    _, apps, _ = run_text([str(tool), "--listapps"], timeout=20)
    if apps:
        results.append(finding("INFO", "firewall", "Règles applicatives firewall", limit_lines(apps, 20)))
    return results
...
def check_sharing() -> list[Finding]:
    ...
    if not selected:
        return [finding("OK", "sharing", "Aucun service de partage distant surveillé détecté")]
    return [finding("WARNING", "sharing", "Services de partage distant à vérifier", "\n".join(selected[:20]), "Vérifie les services activés dans Réglages Système > Général > Partage.")]
...
def check_network() -> list[Finding]:
    ...
    if listeners:
        ...
        results.append(finding("INFO", "network", f"Ports TCP en écoute : {count}", limit_lines(listeners, 20)))
    else:
        results.append(finding("INFO", "network", "Aucun port TCP en écoute détecté"))
    _, connections, _ = run_text(["lsof", "-nP", "-iTCP", "-sTCP:ESTABLISHED"], timeout=15)
    if connections:
        count = max(0, len(connections.splitlines()) - 1)
        results.append(finding("INFO", "network", f"Connexions TCP établies : {count}", limit_lines(connections, 20)))
    else:
        results.append(finding("INFO", "network", "Aucune connexion TCP établie détectée"))
    ...
    if proxy:
        ...
        results.append(finding(level, "network", "Configuration proxy", limit_lines(proxy, 20), "Vérifie les proxies configurés si tu ne les as pas ajoutés volontairement."))
    return results
...
def check_persistence() -> list[Finding]:
    ...
    if plist_paths:
        results.append(finding("INFO", "persistence", f"LaunchAgents/Daemons tiers : {len(plist_paths)}", "\n".join(str(path) for path in plist_paths[:30])))
    else:
        results.append(finding("OK", "persistence", "Aucun LaunchAgent/Daemon tiers détecté"))
    suspicious = [path for path in plist_paths if suspicious_plist(path)]
    if suspicious:
        results.append(finding("WARNING", "persistence", "Commandes suspectes dans des plist", "\n".join(str(path) for path in suspicious[:30]), "Inspecte ProgramArguments, propriétaire et signature des binaires référencés."))
    startup = [path for path in shell_startup_files() if path.exists()]
    if startup:
        details = "\n".join(f"{path} | {mode_owner(path)}" for path in startup)
        results.append(finding("INFO", "persistence", "Fichiers de démarrage shell présents", details))
    return results


def check_services() -> list[Finding]:
    if not available("brew"):
        return [finding("INFO", "services", "Homebrew non installé")]
    results: list[Finding] = []
    _, services, _ = run_text(["brew", "services", "list"], timeout=20)
    if services:
        results.append(finding("INFO", "services", "Services Homebrew", limit_lines(services, 30)))
    ...
    if custom:
        results.append(finding("WARNING", "services", "Taps Homebrew non standards", "\n".join(custom), "Supprime les taps que tu ne reconnais pas ou que tu n'utilises plus."))
    else:
        results.append(finding("OK", "services", "Taps Homebrew standards"))
    return results


def check_updates(*, deep: bool = False) -> list[Finding]:
    if not deep:
        return [finding("INFO", "updates", "Recherche de mises à jour non lancée", "Utilise --deep pour lancer softwareupdate -l.")]
    code, output, stderr = run_text(["softwareupdate", "-l"], timeout=120)
    text = output or stderr
    if code and not text:
        return [finding("ERROR", "updates", "Recherche de mises à jour impossible")]
    if "No new software available" in text:
        return [finding("OK", "updates", "Aucune mise à jour macOS disponible", limit_lines(text, 20))]
    return [finding("INFO", "updates", "Mises à jour macOS à vérifier", limit_lines(text, 40))]
...
def check_filesystem() -> list[Finding]:
    ...
    if not ssh_dir.exists():
        results.append(finding("INFO", "filesystem", "Aucun dossier ~/.ssh détecté"))
    else:
        loose: list[str] = []
        ...
        if loose:
            results.append(finding("WARNING", "filesystem", "Clés locales avec permissions larges", "\n".join(loose), "Réduis les permissions avec chmod 600 sur les clés privées."))
        else:
            results.append(finding("OK", "filesystem", "Permissions des clés locales restrictives"))
    writable = world_writable_paths()
    if writable:
        results.append(finding("WARNING", "filesystem", "Dossiers sensibles world-writable", "\n".join(str(path) for path in writable[:30]), "Vérifie propriétaire et permissions de ces dossiers."))
    else:
        results.append(finding("OK", "filesystem", "Aucun dossier sensible world-writable détecté"))
    return results
...
def check_processes() -> list[Finding]:
    ...
    if cpu:
        ...
        results.append(finding("INFO", "processes", "Top mémoire", "\n".join([cpu.splitlines()[0], *rows[:10]])))
    return results


def check_docker() -> list[Finding]:
    if not available("docker"):
        return [finding("INFO", "docker", "Docker non installé")]
    code, _, _ = run_text(["docker", "info"], timeout=15)
    if code:
        return [finding("WARNING", "docker", "Docker installé mais daemon indisponible")]
    results = [finding("OK", "docker", "Docker daemon actif")]
    _, containers, _ = run_text(["docker", "ps", "-a", "--format", "table {{.Names}}\t{{.Image}}\t{{.Status}}"], timeout=20)
    if containers:
        results.append(finding("INFO", "docker", "Conteneurs Docker", limit_lines(containers, 30)))
    return results
...
def run_checks(checks: Iterable[str] = DEFAULT_CHECKS, *, deep: bool = False) -> list[Finding]:
    ...
    for check in checks:
        try:
            ...
            raise EclipseError(f"Check inconnu : {check}") from error
    return results
...
def write_report(findings: list[Finding], destination: Path | None = None) -> Path:
    ...
    try:
        ...
        raise EclipseError(f"Impossible d'écrire le rapport sécurité : {error}") from error
    return path


def format_findings(findings: list[Finding]) -> str:
    lines = [f"Score sécurité : {security_score(findings)}/100", f"Résumé : {summary(findings)}", ""]
...
def limit_lines(text: str, limit: int) -> str:
    ...
    if len(lines) <= limit:
        return text
    return "\n".join(lines[:limit] + [f"... {len(lines) - limit} lignes masquées"])
