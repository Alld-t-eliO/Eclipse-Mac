...
def prompt(label: str = "Choice") -> str:
...
def pulse_online(online: bool = True) -> str:
    return (
        f"{success('●')} {paint('LOCAL TARGET ONLINE', GREEN, bold=True)}"
        if online
        else f"{danger('●')} {paint('LOCAL TARGET UNREACHABLE', RED, bold=True)}"
    )
