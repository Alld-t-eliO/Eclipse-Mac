    ...
    for raw in values:
        for item in raw.split(","):
            ...
            if len(tag) > MAX_TAG_LENGTH:
                raise EclipseError(f"Tag trop long : {tag[:24]}…")
            if any(character.isspace() for character in tag):
                raise EclipseError(f"Tag invalide avec espace : {tag}")
            if tag not in seen:
                seen.add(tag)
                tags.append(tag)
    return tuple(tags)
...
def add_memory(
    ...
    *,
    ...
    cleaned = text.strip()
    if not cleaned:
        raise EclipseError("La mémoire ne peut pas être vide.")
    if len(cleaned) > MAX_TEXT_LENGTH:
        raise EclipseError(f"Mémoire trop longue : maximum {MAX_TEXT_LENGTH} caractères.")
    entry = MemoryEntry(
...
def write_entry(entry: MemoryEntry, path: Path) -> None:
    try:
        ...
        raise EclipseError(f"Impossible d'écrire la mémoire : {error}") from error
        ...
        with memory_path.open("r", encoding="utf-8") as stream:
            for index, line in enumerate(stream, start=1):
                ...
                try:
                    ...
                    raise EclipseError(f"Mémoire illisible ligne {index} : {error}") from error
                if not isinstance(record, dict):
                    raise EclipseError(f"Mémoire illisible ligne {index} : objet attendu.")
                entry = MemoryEntry.from_record(record)
                if entry.id and entry.text:
                    entries.append(entry)
    except OSError as error:
        raise EclipseError(f"Impossible de lire la mémoire : {error}") from error
    return entries
...
def export_json(destination: Path, *, path: Path | None = None) -> Path:
    ...
    try:
        ...
        raise EclipseError(f"Impossible d'exporter la mémoire : {error}") from error
    return target
